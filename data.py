...
class LlavaDataset(Dataset):
    ...
    def build_dataset(self, data_dir: str) -> Tuple[List[Dict], Path]:
        ...
        assert os.path.exsits(data_dir)
    ...
    def __getitem__(self, idx:int) -> Tuple[str, str, Path]:
        ...
        chatbot_output = conversations[0].get('value')
...
class LlavaDataCollector:
    def __init__(self, processor:AutoProcessor, max_length:int, IGNORE_ID:int=-100,):
    ...
    def convert_one(self, q_input_ids:torch.Tensor, a_input_ids:torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        input_ids = torch.cat([q_input_ids, a_input_ids, self.eos_id], dim=-1)
        labels = torch.cat(
            (torch.full((q_input_ids.size(0),), self.ignore_id, dtype=torch.long),
            a_input_ids, self.eos_id), dim=-1)
    ...
    def __call__(self, batch:List[Any]) -> Dict[str: torch.Tensor]:
        ...
        input_ids = torch.stack(
            [torch.cat([torch.full((max_length - ids.size(0),), self.pad_id, dtype=torch.long), ids]) for ids in input_id_list]
        )
        labels = torch.stack(
            [torch.cat([torch.full((max_length - ids.size(0),), self.ignore_id, dtype=torch.long), ids]) for ids in labels_list]
        )
